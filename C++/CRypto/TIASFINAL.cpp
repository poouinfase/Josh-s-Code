...
public:
  Sha512ByteArray(int le) : WordArray(le) { assert(0 == (le % 128)); }
...
public:
  ...
  void out() {
    ByteArray->print();
    std::cout << "Final hash value is: " << hash() << std::endl;
  }
...
public:
  MD5ByteArray(uint64_t le) : WordArray(le) { assert(0 == (le % 64)); }

  void procBlock(uint64_t ind) {
    uint32_t AA = A, BB = B, CC = C, DD = D;

    uint32_t X[16];
    for (int i = ind; i < ind + MD5BLOCKSIZE; i++) {
  ...
  std::string transform() {
    for (int i = 0; i < len; i += MD5BLOCKSIZE) {
      procBlock(i);
    }
    std::stringstream strout;
...
public:
  ...
  std::string transform() {
    ...
    std::stringstream strout;
    strout << std::setfill('0') << std::setw(8) << std::hex << h0;
    strout << std::setfill('0') << std::setw(8) << std::hex << h1;
    strout << std::setfill('0') << std::setw(8) << std::hex << h2;
    strout << std::setfill('0') << std::setw(8) << std::hex << h3;
    strout << std::setfill('0') << std::setw(8) << std::hex << h4;
    return strout.str();
  }
};
...
public:
  ...
  std::pair<uint64_t, uint64_t> SignProd(std::string msg) {
    ...
    HashGen(msg);
    secretNum = 3;
    std::cout << "Please give a secret number(k): ";
    std::cin >> secretNum;
    auto [s, r] = signGet();
  ...
  int verifyMath(uint64_t *pub, uint64_t r, uint64_t s, DSS S) {
    ...
    uint64_t v =
        fmod(fmod(powAh(pub[2], u1) * powAh(pub[3], u2), pub[0]), pub[1]) + r;
...
int DssEg(std::string message) {
  srand(time(NULL));
  uint64_t p, q, h, x, HM;
  std::cout << "Please provide prime number p:\n";
  std::cin >> p;
  std::cout << "Please provide prime number q\n";
  std::cin >> q;
  std::cout << "Please provide the helper integer h\n";
  std::cin >> h;
  std::cout << "Please provide the private x\n";
  std::cin >> x;
  std::cout << "Provide HM: ";
  std::cin >> HM;
  ...
  if (temp.verify(temp, sRec, rRec)) {
    std::cout << "MESSAGE (\"" << message << "\") is VERIFIED" << std::endl;
  } else {
    std::cout << "MESSAGE (\"" << message << "\") is NOT VERIFIED" << std::endl;
  }
  return 0;
}
